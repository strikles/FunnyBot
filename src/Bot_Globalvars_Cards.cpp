...
bool CGlobalVars::CardsDealt(const holdem_state& curState)
{
	/*
...
void CGlobalVars::set_card_values()
{
	...
	*/
	...
	//pocket cards
	Bot_Common_Cards[0][0] = RANK(CurState().m_player[userchair].m_cards[0]);
	Bot_Common_Cards[0][1] = SUIT(CurState().m_player[userchair].m_cards[0]);
	Bot_Common_Cards[1][0] = RANK(CurState().m_player[userchair].m_cards[1]);
	Bot_Common_Cards[1][1] = SUIT(CurState().m_player[userchair].m_cards[1]);
	...
	for(int i=0;i<5;i++)
	{
		Bot_Common_Cards[i+2][0] = RANK(CurState().m_cards[i]);
		Bot_Common_Cards[i+2][1] = SUIT(CurState().m_cards[i]);

		if (!ISUNKNOWN(CurState().m_cards[i])) 
		{
...
bool CGlobalVars::isHand(const char* handslist)
{
	...
	*/
	...
	//gLog->WriteLog(eSeverityInfo, eCatDebug, "My Hand is: %s \n", gVars.myHand );
	//gLog->WriteLog(eSeverityInfo, eCatDebug, "Hands to search: %s \n",handslist);

	if(strstr(handslist, myHand) != NULL)
	{
		//gLog->WriteLog(eSeverityInfo, eCatDebug, "Hand found! \n");
		//Hand found
		return 1;
	}

	//gLog->WriteLog(eSeverityInfo, eCatDebug, "Hand not found :( \n");
