...
int CGlobalVars::set_raischair()
{
	...
	*/
	...
	for (int i=0; i < k_MaxChairs; i++)
	{
		if (IsEqual(currentbets[i], ncurrentbet) && currentbets[i] > check_currentbet(previous_player(get_ac_dealpos(i))))
			return i;
	}
...
int CGlobalVars::set_preflop_position()
{
	...
	* 0 - error , 1 to 6 valid preflop position (SB=1, BB=2, UTG=3, HJ=4, CO=5, BT=6)
	*/
...
int CGlobalVars::opponent_position(int opponent_chair)
{
	...
	double pre_adjusted_position = 0;

	if(opponent_chair >= 0 && opponent_chair < k_MaxChairs)
	{
...
int CGlobalVars::pf_raiser_position() 
{
	...
	*/
	...
	if(gVars.get_ncallbets() < 1)
		return -1; //no raiser
	if (gVars.get_nplayersdealt() == 6 && gVars.dealpositionrais == gVars.get_nplayersdealt()-3) 
		return 3; //UTG
	if (gVars.get_nplayersdealt() >= 5 && gVars.dealpositionrais == gVars.get_nplayersdealt()-2) 
		return 4; //HJ
	if (gVars.get_nplayersdealt() >= 4 && gVars.dealpositionrais == gVars.get_nplayersdealt()-1) 
		return 5; //CO
	if (gVars.get_nplayersdealt() >= 3 && gVars.dealpositionrais == gVars.get_nplayersdealt()) 
		return 6; //BTN
